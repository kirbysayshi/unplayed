import { useDb, StatusKeys, query } from "./aodb.js";
import { stract } from "./stract.js";
export var Actions;
(function (Actions) {
    Actions[Actions["ComputeResult"] = 0] = "ComputeResult";
    Actions[Actions["EditEntity"] = 1] = "EditEntity";
    Actions[Actions["NewEntity"] = 2] = "NewEntity";
})(Actions || (Actions = {}));
export const dispatch = (action, entityId) => {
    switch (action) {
        ...
        case Actions.EditEntity: {
            if (!entityId)
                throw new Error('EntityId was not received!');
...
export function EntityPanel(ghHref) {
    // const panel = document.createElement("div");
    // panel.setAttribute("data-compose", "");
    return stract `
  `;
}
function EditEntityPanel(ghHref, entity) {
    ...
    <label>
      Status
      <select name="status">
        ${StatusKeys.map(name => `<option value="${name}">${name}</option>`)}
      </select>
    </label>
    <label>
      Game Name
